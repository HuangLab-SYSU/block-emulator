...
func data2tx(data []string, nonce uint64) (*core.Transaction, bool) {
	if data[6] == "0" && data[7] == "0" && len(data[3]) > 16 && len(data[4]) > 16 && data[3] != data[4] {
		...
		// For cross-shard transactions, check whether any broker can afford the value.
		// A single broker's maximum spendable balance is Init_broker_Balance * ShardNum
		// (initial balance spread across all shards). If the tx value exceeds this cap,
		// no broker will ever be able to handle it, so skip the transaction entirely.
		sender, recipient := data[3][2:], data[4][2:]
		if utils.Addr2Shard(sender) != utils.Addr2Shard(recipient) {
			maxBrokerVal := new(big.Int).Mul(
				params.Init_broker_Balance,
				big.NewInt(int64(params.ShardNum)),
			)
			if val.Cmp(maxBrokerVal) > 0 {
		...
		tx := core.NewTransaction(sender, recipient, val, nonce, fee)
		return tx, true
	}
	return &core.Transaction{}, false
}
