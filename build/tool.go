...
func Exebat_Windows_GenerateBatFile(nodenum, shardnum, modID int) {
	...
	for i := 1; i < nodenum; i++ {
		for j := 0; j < shardnum; j++ {
			str := fmt.Sprintf("start cmd /k blockEmulator_Windows_Precompile.exe -n %d -N %d -s %d -S %d -m %d \n\n", i, nodenum, j, shardnum, modID)
			ofile.WriteString(str)
		}
	}

	for j := 0; j < shardnum; j++ {
		str := fmt.Sprintf("start cmd /k blockEmulator_Windows_Precompile.exe -n 0 -N %d -s %d -S %d -m %d \n\n", nodenum, j, shardnum, modID)
		ofile.WriteString(str)
	}

	str := fmt.Sprintf("start cmd /k blockEmulator_Windows_Precompile.exe -c -N %d -S %d -m %d \n\n", nodenum, shardnum, modID)

	ofile.WriteString(str)
}

func Exebat_Linux_GenerateShellFile(nodenum, shardnum, modID int) {
	fileName := fmt.Sprintf("bat_shardNum=%v_NodeNum=%v_mod=%v.sh", shardnum, nodenum, params.CommitteeMethod[modID])
	ofile, err := os.OpenFile(fileName, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
	if err != nil {
		log.Panic(err)
...
func Exebat_MacOS_GenerateShellFile(nodenum, shardnum, modID int) {
	fileName := fmt.Sprintf("bat_shardNum=%v_NodeNum=%v_mod=%v.sh", shardnum, nodenum, params.CommitteeMethod[modID])
	ofile, err := os.OpenFile(fileName, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
	if err != nil {
		log.Panic(err)
	...
	for j := 0; j < shardnum; j++ {
		for i := 1; i < nodenum; i++ {
			str := fmt.Sprintf("./blockEmulator_MacOS_Precompile -n %d -N %d -s %d -S %d -m %d &\n\n", i, nodenum, j, shardnum, modID)
			ofile.WriteString(str)
		}
	}

	for j := 0; j < shardnum; j++ {
		str := fmt.Sprintf("./blockEmulator_MacOS_Precompile -n 0 -N %d -s %d -S %d -m %d &\n\n", nodenum, j, shardnum, modID)
		ofile.WriteString(str)
	}

	str := fmt.Sprintf("./blockEmulator_MacOS_Precompile -c -N %d -S %d -m %d &\n\n", nodenum, shardnum, modID)
